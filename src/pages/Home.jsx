      ...
      <section className="grid grid-cols-1 lg:grid-cols-8 gap-1 lg:gap-4" onTouchStart={startLongPress} onTouchEnd={cancelLongPress}>
        <div className={`card py-2 px-4 lg:col-span-3 flex flex-col justify-center min-h-[85px] lg:min-h-[150px] relative ${editMode ? 'animate-[wiggle_0.3s_ease-in-out_infinite]' : ''}`}>
          ...
          <div className="mt-1 flex items-center gap-2 flex-wrap">
            ...
            {snapshotMsg && (
              <span className={`text-xs truncate ${snapshotMsg.type === 'error' ? 'text-red-500' : snapshotMsg.type === 'warn' ? 'text-yellow-600' : 'text-green-600'}`}>
                {snapshotMsg.text}
              </span>
            )}
          </div>
      ...
      {cardConfig.trend && (
        <div className={editMode ? 'animate-[wiggle_0.3s_ease-in-out_infinite]' : ''}>
          <div className="relative">
            ...
            {editMode && <button onClick={() => toggleCard('trend')} className="absolute top-2 right-2 w-5 h-5 rounded-full bg-red-500 text-white text-xs flex items-center justify-center shadow z-10">−</button>}
          </div>
        </div>
      )}
      ...
      {cardConfig.allocation && (
        <div className={editMode ? 'animate-[wiggle_0.35s_ease-in-out_infinite]' : ''}>
          <div className="relative">
            ...
            {editMode && <button onClick={() => toggleCard('allocation')} className="absolute top-2 right-2 w-5 h-5 rounded-full bg-red-500 text-white text-xs flex items-center justify-center shadow z-10">−</button>}
          </div>
        </div>
      )}
      ...
      {cardConfig.holdings && (
        <div className={editMode ? 'animate-[wiggle_0.4s_ease-in-out_infinite]' : ''}>
          <div className="relative">
            ...
            {editMode && <button onClick={() => toggleCard('holdings')} className="absolute top-2 right-2 w-5 h-5 rounded-full bg-red-500 text-white text-xs flex items-center justify-center shadow z-10">−</button>}
          </div>
        </div>
      )}
