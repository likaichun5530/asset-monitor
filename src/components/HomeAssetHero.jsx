    ...
    <div className="card home-hero-card flex min-h-[92px] flex-col justify-center overflow-hidden px-4 py-3 sm:min-h-[184px] sm:p-0">
      <div className="flex min-h-[68px] items-stretch justify-between gap-3 sm:min-h-[110px] sm:gap-8">
        <div className="flex min-w-0 flex-1 flex-col justify-center sm:px-8 sm:py-7 lg:px-10">
          ...
          <div className="font-num mt-1 whitespace-nowrap text-[28px] font-bold leading-none tracking-[-0.04em] text-gray-900 dark:text-gray-100 sm:mt-4 sm:text-[42px] sm:text-white lg:text-[46px]">
            {valuesHidden ? '******' : formatNumber(total)}
          </div>
          <div className="font-num-regular mt-1.5 text-xs text-gray-400 sm:mt-4 sm:text-slate-400">更新于 {updateDate ? formatDateLong(updateDate) : '--'}</div>
        </div>

        <div className="flex shrink-0 flex-col items-end justify-center text-right sm:min-w-[230px] sm:border-l sm:border-white/10 sm:bg-white/[0.035] sm:px-8 lg:min-w-[260px]">
          <button type="button" onClick={onOpenTodayDetail} disabled={valuesHidden} aria-label={valuesHidden ? '资产金额已隐藏' : '查看今日盈亏明细'} className="font-num flex flex-col items-end rounded-lg bg-transparent text-right transition-transform active:scale-[0.99] disabled:cursor-default disabled:active:scale-100">
            <span className="text-xs font-medium text-gray-400 sm:text-slate-300">今日盈亏</span>
            <span className={`mt-1 text-base font-medium leading-none sm:mt-3 sm:text-[22px] ${changeColor}`}>
              {valuesHidden ? '******' : todayChange === null || todayChange === undefined ? '--' : formatChange(todayChange)}
            </span>
            <span className={`mt-1 text-xs font-medium sm:mt-2 ${changeColor}`}>
              {valuesHidden ? '******' : todayChangePct === null || todayChangePct === undefined ? '--' : formatPercent(todayChangePct, { withSign: true })}
            </span>
          </button>
          <button type="button" onClick={onToggleEdit} className="mt-4 hidden h-8 items-center gap-1.5 rounded-lg border border-white/15 bg-white/[0.07] px-3 text-xs font-medium text-slate-300 transition-colors hover:border-white/30 hover:bg-white/[0.12] hover:text-white sm:inline-flex">
      ...
      {pendingCount > 0 && <div className="mt-1 flex flex-wrap items-center gap-2 sm:absolute sm:bottom-4 sm:left-8"><span className="text-xs text-yellow-600 sm:text-amber-300">{pendingCount} 条待同步</span></div>}
    </div>
  )
}
