...
export default function HomeAssetHero({ total, updateDate, pendingCount, snapshotMsg, snapshotLoading, editMode, onToggleEdit, onSnapshot }) {
  return (
    <div className="card home-hero-card py-2 px-4 sm:p-5 flex flex-col justify-center min-h-[85px] sm:min-h-[150px] relative overflow-hidden">
      <div>
        <div className="flex items-start justify-between gap-6 sm:relative">
          ...
          <div className="static flex shrink-0 items-center gap-2 sm:relative">
            ...
            <button onClick={onSnapshot} disabled={snapshotLoading} className="absolute right-0 top-0 inline-flex h-[54px] w-[54px] shrink-0 items-center justify-center gap-2 rounded-xl bg-transparent text-gray-700 transition-colors hover:bg-gray-50 disabled:opacity-60 dark:text-gray-100 dark:hover:bg-gray-700 sm:static sm:h-10 sm:w-auto sm:rounded-xl sm:border sm:border-gray-200 sm:px-4 sm:text-sm sm:font-medium dark:sm:border-gray-600">
              {snapshotLoading ? (
                ...
                <SnapshotIcon className="h-[30px] w-[30px] sm:h-5 sm:w-5" />
              )}
