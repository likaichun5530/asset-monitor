...
- 重排桌面首页：总资产与资产趋势并排，今日盈亏移至总资产数值下方；货币比例、账户健康度和收益日历固定为同一行，其余分析卡保持双列。
- 放大桌面端全局卡片正文、标题和表格字号，改用更舒展的常规数字字面；重点放大四张盈亏卡的金额、比例和标签，并同步提高日历辅助文字、保证金刻度及货币圆环的可读性。
- 本次仅调整 1024px 及以上桌面布局，手机端布局保持不变。
...
- 重构桌面端视觉系统：采用深墨色侧栏、收紧的工作台顶栏与统一的 1480px 内容网格，重新设计导航选中态、状态控件、按钮、卡片边框及明暗模式层级。
- 将桌面总资产升级为主视觉面板，分区呈现总资产与今日盈亏；统一首页指标、分析卡间距并降低非重点卡片的阴影干扰。移动端布局和业务功能保持不变。
