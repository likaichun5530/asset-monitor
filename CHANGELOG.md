...
- 收益日历按月份实际需要生成 5 行；仅在日期无法完整容纳时使用 6 行，并自动压缩行高。日期与收益改为上下排布，避免文字重叠。
- 放大桌面货币比例的圆环、图例和文字，以及账户健康度的评分、配置建议、保证金信息和风险图例，使内容随卡片空间舒展显示。
- 桌面左侧资产账户改为可折叠下拉菜单；进入任一资产页时自动展开。修正左上角应用 Logo 的裁切问题，完整显示图标。
