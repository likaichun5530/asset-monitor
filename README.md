...
- 美股和港股详情采用账户口径，包含对应市场的股票和现金；A股详情只统计 A 股股票。现金在资产配置口径中仍统一归入现金，独立现金使用 `Market = GLOBAL`
...
每日一条 `{ date, total, categories?, note? }` 快照。手动和定时快照都会从 `Holdings` 实时汇总总资产与九类资产数据；美股和港股历史按账户口径包含对应市场现金，A股历史只包含股票（现金列仍保留全部现金，总资产只累计一次），同日已有记录时覆盖并保留原备注，否则追加。
