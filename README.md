...
- 美股详情采用账户口径，包含 `Market = US` 的美股股票和现金；现金在资产配置口径中仍归入现金
...
每日一条 `{ date, total, categories?, note? }` 快照。手动和定时快照都会从 `Holdings` 实时汇总总资产与九类资产数据；美股历史从此版本起按账户口径包含 `Market = US` 的现金（现金列仍保留该金额），同日已有记录时覆盖并保留原备注，否则追加。
