...
- 美股详情采用账户口径，包含美股股票和 US 现金；A股和港股详情只统计对应市场股票。现金在资产配置口径中仍统一归入现金，其他现金（非证券账户）使用 `Market = GLOBAL`
- 证券账户现金名称自动使用“人民币现金 / 美元现金 / 港币现金”；“其他现金（非证券账户）”用于银行卡、活期存款和备用金，使用 `Market = GLOBAL`
