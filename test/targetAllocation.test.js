...
test('目标偏差仅在超出容忍范围时提醒，边界本身不触发', () => {
  assert.equal(getTargetAllocationStatus(0.12, 0.10).status, 'balanced')
  assert.equal(getTargetAllocationStatus(0.1201, 0.10).status, 'over')
...
test('目标容忍范围取目标的40%，下限0.5、上限2个百分点', () => {
  ...
  assert.deepEqual(getTargetAllowedRange(0), { lower: 0, upper: 0.005 })
  assert.equal(getTargetAllocationStatus(0.005, 0).status, 'balanced')
  assert.equal(getTargetAllocationStatus(0.0051, 0).status, 'over')
})
...
test('手机目标页同时表达实际配置、计划目标和调整方向', async () => {
  ...
  assert.match(source, /偏离目标范围尺/)
  assert.match(source, /容忍幅度为目标比例的 40%，最低 0.5、最高 2 个百分点/)
  assert.match(source, /低配线/)
  assert.match(source, /超配线/)
