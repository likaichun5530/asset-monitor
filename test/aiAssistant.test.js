...
test('AI 模型清单接口要求登录，设置页提供增删编辑入口', async () => {
  ...
  assert.match(settingsSource, /h-\[calc\(100dvh-8px\)\]/)
  assert.match(settingsSource, /document\.body\.style\.position = 'fixed'/)
  assert.match(settingsSource, /window\.scrollTo\(0, scrollY\)/)
