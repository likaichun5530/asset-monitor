...
test('AI入口位于标题栏用户名左侧，弹窗锁定页面并由返回键优先关闭', async () => {
  ...
  assert.match(layoutSource, /compact \? 'h-6 w-6' : 'h-8 w-8'/)
  assert.match(layoutSource, /strokeWidth="2\.8"/)
  assert.match(layoutSource, /WebkitTapHighlightColor: 'transparent'/)
  assert.match(aiButtonSource, /text-slate-950 dark:text-white/)
  assert.doesNotMatch(aiButtonSource, /text-brand-/)
...
test('AI 模型清单接口要求登录，设置页提供增删编辑入口', async () => {
  ...
  assert.match(dialogSource, /<footer className=/)
