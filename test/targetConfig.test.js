...
test('目标页面提供统一弹窗编辑入口并通过现有 target 接口保存', async () => {
  ...
  assert.match(dialog, /sortedCategories/)
  ...
  assert.match(detailDialog, /isOtherItem/)
  assert.match(detailDialog, /evaluatedItems = detailItems\.filter/)
  assert.match(detailDialog, /otherItem \? <div/)
  ...
  assert.match(detailConfigDialog, /sort\(\(a, b\) => b\.targetRatio - a\.targetRatio\)/)
